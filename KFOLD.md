...
|---|---|---|---|---|---|---|---|---|
| 1 | `cd_only_f0` | CD | 259（最优 239） | ✅ 早停 | 7 | 0.0089 | 6.455 | ✅ |
| 2 | `lr_fix_only_f0` | CD+DCD | 230（最优 210） | ✅ 早停 | 7 | 0.0120 | 6.393 | ✅ |
| 3 | `rep_w05_f0` | CD+DCD+rep | | | | | | |
| 4 | `cd_rep05_full_f0` | CD+rep ⭐ | | | | | | |
| 5 | `cd_only_f1` | CD | 275（最优 255） | ✅ 早停 | 9 | 0.0059 | 6.479 | ✅ |
| 6 | `lr_fix_only_f1` | CD+DCD | 250（最优 230） | ✅ 早停 | 7 | 0.0147 | 6.447 | ✅ |
| 7 | `rep_w05_f1` | CD+DCD+rep | | | | | | |
| 8 | `cd_rep05_full_f1` | CD+rep ⭐ | | | | | | |
| 9 | `cd_only_f2` | CD | 250（最优 230） | ✅ 早停 | 8 | 0.0038 | 6.310 | ✅ |
| 10 | `lr_fix_only_f2` | CD+DCD | 207（最优 187） | ✅ 早停 | 6 | 0.0139 | 6.247 | ✅ |
| 11 | `rep_w05_f2` | CD+DCD+rep | | | | | | |
| 12 | `cd_rep05_full_f2` | CD+rep ⭐ | | | | | | |
| 13 | `cd_only_f3` | CD | 269（最优 249） | ✅ 早停 | 9 | 0.0030 | 6.301 | ✅ |
| 14 | `lr_fix_only_f3` | CD+DCD | 306（最优 286） | ✅ 早停 | 8 | 0.0039 | 6.292 | ✅ |
| 15 | `rep_w05_f3` | CD+DCD+rep | | | | | | |
| 16 | `cd_rep05_full_f3` | CD+rep ⭐ | | | | | | |
| 17 | `cd_only_f4` | CD | 241（最优 221） | ✅ 早停 | 8 | 0.0069 | 6.529 | ✅ |
| 18 | `lr_fix_only_f4` | CD+DCD | 353（最优 333） | ✅ 早停 | 9 | 0.0032 | 6.493 | ✅ |
| 19 | `rep_w05_f4` | CD+DCD+rep | | | | | | |
| 20 | `cd_rep05_full_f4` | CD+rep ⭐ | | | | | | |

**已跑完两个模型（10/20）**：

| 配置 | run.json CD_t 折均值 | 主指标 `defect_cov_mm` 折均值 | 扎堆率 |
|---|---|---|---|
| `cd_only` | 6.415 ± 0.103 | 3.438 ± 0.061 | 14.54% |
| `lr_fix_only` | 6.374 ± 0.103 | **3.298** ± 0.137 | **5.81%** |

⭐ 2×2 第一格（**+DCD，无 rep**）：`defect_cov_mm` delta **−0.140**、fold SE 0.042、
**5/5 同向**、CI [−0.255, −0.025] 不跨零 → 按「同向 且 |delta| > 2×SE」**成立**。
单折那格是 −0.197，**k 折缩水 29%** —— 方向与 2026-08-05 写下的预期一致。

⚠️ 同一格在全点云 `CD_t` 上 CI **跨零**（−0.032，[−0.067,+0.003]，4/5）——
**缺损区分得出、全点云分不出**，这是主指标选择的 k 折版证据。

⚠️ 配对能抵消多少方差**取决于指标**：两配置逐折相关 `CD_t` r=+0.99（抵消 6 倍）、
`defect_cov_mm` r=+0.82、`clump_%` r=−0.06（完全不抵消）。
**主指标的配对 SE 是 0.042**，别拿 `CD_t` 的 0.013 去估它。
