...
| **`rep_w05`** | `--repulsion-weight 0.5`（无量纲化后）+ 修好的学习率衰减。<br>**扎堆 12.1% → 0.9%，且 CD_t / 偏差 p95 / 偏差 std 同时最好。**<br>⚠️ 同时改了两件事，CD_t 的增益归因未拆开。 | **6.326 mm** | 222（最优在 220） |
...
密度指标（扎堆率、间距 CV）不受这个问题困扰 —— `rep_w05` 的 12.1% → 0.9% 远超任何
方差解释，是目前**唯一一个可以确定性下结论**的改进。
