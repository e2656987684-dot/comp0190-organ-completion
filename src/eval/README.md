...
| **`mesh_preview.py`** | `$PY src/eval/mesh_preview.py`<br>（`--run` / `--skull` / `--res` / `--truth` 加原始体数据两格 / `--camera` / ⚠️`--preset` 仅探索用） | **PNG，编辑器里直接打开** | ✅ 写 `reports/preview/<run>_<skull>.png`（**gitignored**，约 1 MB）；加 `--html` 才另写可旋转的 HTML（20~40 MB，⚠️ VSCode 预览不了） | ✅ 建一个 187M 模型 | 约 1 分钟 |
